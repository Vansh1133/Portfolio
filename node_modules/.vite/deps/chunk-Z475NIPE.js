...
function deepExtend(destination, ...sources) {
  for (const source of sources) {
    ...
    const sourceKeys = Object.keys(source), dangerousKeys = /* @__PURE__ */ new Set(["__proto__", "constructor", "prototype"]), hasNested = sourceKeys.some((k) => {
      const v = source[k];
...
function getStyleFromHsl(color, hdr, opacity) {
  ...
  return getCachedStyle(key, () => hdr ? getStyleFromRgb(hslToRgb(color), true, opacity) : `hsla(${color.h.toString()}, ${color.s.toString()}%, ${color.l.toString()}%, ${op.toString()})`);
}
...
function updateColorValue(data, decrease, delta) {
  ...
  const offset = data.offset ? randomInRangeValue(data.offset) : minOffset, velocity = (data.velocity ?? minVelocity2) * delta.factor + offset * velocityFactor, decay = data.decay ?? identity2, max = data.max, min = data.min;
...
//# sourceMappingURL=chunk-Z475NIPE.js.map
